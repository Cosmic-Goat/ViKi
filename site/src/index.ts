...
const getData = async (file: string) => new Uint32Array(await (await (await fetch(file)).blob()).arrayBuffer());

const bpmData = await getData("bpm.dat")
const tempData = getData("temp.dat");
const oxData = getData("ox.dat");
